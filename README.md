...
Xcash 是一个面向商家和平台方的加密货币支付基础设施，适合需要接入加密货币收款、USDT 收款、链上充值和提币能力的业务系统。项目支持 EVM 兼容链和 Bitcoin、Tron 等网络，提供支付、充值、提币、自动归集等完整的加密货币金融网关能力。
系统完全自托管，支付场景下资金可直接进入商户自己的链上收款地址，资产安全永远是第一驱动力。

## 核心功能
- 支付收款；支持 USDT 收款和多种加密货币收款，买家付款直达您的链上收款地址，平台不托管资金。
- 充提币；允许您的平台用户像交易所用户一样充值和提现代币。
- **兼容易支付（EPay）**；项目创建时系统自动分配 EPay 商户身份（pid + 密钥），无需手动开通即可通过标准易支付 V1 协议下单；typecho/wordpress/discuz 等主流开源系统的易支付插件可直接对接，无需改造插件代码。
