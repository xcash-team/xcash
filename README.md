...
> **重要：充提币功能需要手动开启原生币扫描**
> 
> 系统默认关闭了 EVM 原生币扫描（`open_native_scanner`）。**充值、提币功能依赖原生币扫描才能正常工作**，因为 Gas 分发、归集等链上交易必须通过原生币扫描来感知和确认。
> 
> 开启方法：
> 1. 登录管理后台，进入 **平台运行参数** 页面
> 2. 开启 **「开启 EVM 原生币扫描」** 开关
> 
> **⚠️ EVM 链的 RPC 节点必须支持高频调用。** 原生币扫描需要持续轮询链上区块，服务商提供的免费套餐通常有严格的请求次数限制（如每天几千次），完全无法满足系统的扫描需求。请务必使用支持高频调用的付费 RPC 套餐（如 QuickNode、Alchemy 的 Growth/Scale 套餐，或自建全节点）。
